...
def test_camera_frame_history_uses_recorded_execution_columns(tmp_path: Path):
    data_root = tmp_path / "datasets" / "history_manual"
    names = [
    ...
    first = ((0.0, 0.0),) * 4
    second = ((0.0, 0.0),) * 3 + ((10.0, 20.0),)
    for name in names:
        write_session(
            data_root,
            name,
            labels=[(10.0, 20.0), (12.0, 21.0)],
            control_mode="history_gamepad",
            recorded_histories=[first, second],
        )
    manifest = write_split_manifest(
        tmp_path / "config" / "split.yaml",
        train=[names[0]],
    ...
    attached = attach_executed_command_history(
        build_policy_data_splits(config),
        history_frames=4,
        require_recorded_history=True,
    )

    assert attached.train_samples[0].history_class_ids == ((100, 100),) * 4
    assert attached.train_samples[1].history_class_ids[-1] == (110, 120)


def test_camera_frame_history_rejects_discontinuous_execution_echo(tmp_path: Path):
    data_root = tmp_path / "datasets" / "history_manual"
    names = [
        "20260817_121000_000_session",
        "20260817_121100_000_session",
        "20260817_121200_000_session",
    ]
    first = ((0.0, 0.0),) * 4
    broken = ((0.0, 0.0),) * 3 + ((99.0, 20.0),)
    for name in names:
        write_session(
            data_root,
            name,
            labels=[(10.0, 20.0), (12.0, 21.0)],
            control_mode="history_gamepad",
            recorded_histories=[first, broken],
        )
    manifest = write_split_manifest(
        tmp_path / "config" / "split.yaml",
        train=[names[0]],
    ...
    with pytest.raises(PolicyDatasetError, match="history is discontinuous"):
        attach_executed_command_history(
            build_policy_data_splits(config),
            history_frames=4,
            require_recorded_history=True,
