...
schema v7과 두 policy 공유 CUDA IPC를 지원하는
`xycar/ai-drive:jp6.2.1-pytorch25.06-schema7-traffic-actions3-v8` tag다. 기본 nice_adaptive artifact는
...
grep -Fx 'GPU_IMAGE=xycar/ai-drive:jp6.2.1-pytorch25.06-schema7-traffic-actions3-v8' \
  /home/xytron/.local/lib/xycar-ai-gpu/images.lock.env
docker image inspect xycar/ai-drive:jp6.2.1-pytorch25.06-schema7-traffic-actions3-v8 >/dev/null
```
...
schema v7은 사람 보정 YOLO11s의 640 letterbox 결과에서 confidence `0.25` 이상
...
softmax `0.50` 미만은 `UNKNOWN`이고 3 camera frame마다 한 번 판독한다.
`STOP`은 3회, `LEFT/STRAIGHT`는 15회 연속이어야 확정되며, STOP latch는
동일 `LEFT/STRAIGHT`가 15회 확정될 때까지 유지한다. YOLO box가 10회의
scheduled 판독, 즉 30 camera frame 동안 없으면 기존처럼 STOP latch를
해제한다. shortcut이 실제 motor를 제어하는 동안 Base self-AR shadow를
계속 갱신하되 발행하지 않는다. 8초 종료 때 최신 0.25초 이내 shadow command를
즉시 발행하며, 누락·stale·IPC 오류는 fallback 없이 정지한다. red 취소는 shadow를
폐기한다. 기존 schema v6 `votes2` bundle은 rollback용으로 보존한다.
