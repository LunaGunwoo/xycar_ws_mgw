...
schema v8과 두 policy 공유 CUDA IPC를 지원하는
`xycar/ai-drive:jp6.2.1-pytorch25.06-schema8-traffic-actions3-v9` tag다. 기본 nice_adaptive artifact는
...
schema v8 artifact가 container 시작 직후 종료될 수 있다.
...
grep -Fx 'GPU_IMAGE=xycar/ai-drive:jp6.2.1-pytorch25.06-schema8-traffic-actions3-v9' \
  /home/xytron/.local/lib/xycar-ai-gpu/images.lock.env
docker image inspect xycar/ai-drive:jp6.2.1-pytorch25.06-schema8-traffic-actions3-v9 >/dev/null
```
...
schema v8은 사람 보정 YOLO11s의 640 letterbox 결과에서 confidence `0.25` 이상
...
`STOP`은 3회(30 FPS 기준 약 0.1초), `LEFT/STRAIGHT`는 15회(약 0.5초) 연속이어야
확정되며, STOP latch는
