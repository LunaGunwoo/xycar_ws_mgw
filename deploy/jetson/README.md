...
CUDA IPC를 지원하는
`xycar/ai-drive:jp6.2.1-pytorch25.06-schema7-traffic-actions3-v7` tag다. 기본 nice_adaptive artifact는
...
grep -Fx 'GPU_IMAGE=xycar/ai-drive:jp6.2.1-pytorch25.06-schema7-traffic-actions3-v7' \
  /home/xytron/.local/lib/xycar-ai-gpu/images.lock.env
docker image inspect xycar/ai-drive:jp6.2.1-pytorch25.06-schema7-traffic-actions3-v7 >/dev/null
```
...
schema v6는 사람 보정 YOLO11s의 640 letterbox 결과에서 confidence `0.25` 이상
...
softmax `0.50` 미만은 `UNKNOWN`이고 동일 raw class가 3 frame마다 2회 연속이어야
확정되며, STOP latch는 2회 확정된 `LEFT/STRAIGHT` 전까지 유지한다. shortcut이 실제 motor를 제어하는 동안 Base self-AR shadow를
