...
schema v9와 두 policy 공유 CUDA IPC를 지원하는
`xycar/ai-drive:jp6.2.1-pytorch25.06-schema9-traffic-actions3-v10` tag다. 기본 nice_adaptive artifact는
...
schema v9 artifact가 container 시작 직후 종료될 수 있다.
...
grep -Fx 'GPU_IMAGE=xycar/ai-drive:jp6.2.1-pytorch25.06-schema9-traffic-actions3-v10' \
  /home/xytron/.local/lib/xycar-ai-gpu/images.lock.env
docker image inspect xycar/ai-drive:jp6.2.1-pytorch25.06-schema9-traffic-actions3-v10 >/dev/null
```
...
schema v9는 사람 보정 YOLO11s의 640 letterbox 결과에서 confidence `0.25` 이상
...
`STOP`은 10회(30 FPS 기준 약 0.33초), `LEFT/STRAIGHT`는 15회(약 0.5초) 연속이어야
확정되며, STOP latch는
...
폐기한다. 기존 schema v8 `stop3-go15-search3-classify1`과 schema v6 `votes2`
