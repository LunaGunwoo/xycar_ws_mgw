...
schema v13과 두 policy 공유 CUDA IPC를 지원하는
`xycar/ai-drive:jp6.2.1-pytorch25.06-schema13-traffic-actions3-v14` tag다. 기본 nice_adaptive artifact는
...
schema v13 artifact가 container 시작 직후 종료될 수 있다.
...
grep -Fx 'GPU_IMAGE=xycar/ai-drive:jp6.2.1-pytorch25.06-schema13-traffic-actions3-v14' \
  /home/xytron/.local/lib/xycar-ai-gpu/images.lock.env
docker image inspect xycar/ai-drive:jp6.2.1-pytorch25.06-schema13-traffic-actions3-v14 >/dev/null
```
...
`STOP/LEFT/STRAIGHT`는 처리 완료된 동일 분류 15회 연속이어야 확정되며,
STOP latch는 동일 `LEFT/STRAIGHT`가 15회 확정될 때까지 유지한다. camera 30 Hz와
실제 주행 vote rate는 같지 않다.
YOLO box가 10회의
...
폐기한다. 기존 schema v12 `stop10-go30`, schema v11 `stop30-go30`, schema v9 `stop10-go15`, schema v8
...
재시작한다. `use_gamepad:=true`에서는 빨간불 정지 중에도 A를 계속 누르고 있어야
확정된 다음 신호에서 자동 재출발한다.
