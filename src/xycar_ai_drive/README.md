...
`/xycar_motor`를 독점 발행한다. 현재 bundle schema v20은 schema v6 speed-35
...
- YOLO와 CNN은 같은 fresh frame에서 camera sequence 간격 3 이상마다 함께
  ...
  무시하지만 pending navigation vote는 초기화한다. schema v20에는 no-box 자동
  정지 해제가 없다.
...
- 실제 실행 history와 미발행 Base shadow history는 분리한다. schema v20 Base와
  shadow에는 cap `35`가
...
기존 schema v19 8초 session-rearm, schema v18 process당 1회 fix-Base GO1,
...
ros2 run xycar_ai_drive traffic_shortcut_monitor --ros-args \
  ...
  -p bundle_dir:=/home/xytron/xycar_ws_mgw/artifacts/models/traffic-shortcut-nice-ada-very-fast-fix-speed35-regression-resnet18-4s-shadow-ar-handoff-yolo11s-humanbbox-cnn416-actions3-conf50-tl40to225-initial-wait-stop5-go1-stoponce-leftsession1-search3-classify3-vote-yolo3-t500-45sessions-20260824
```
...
없다. schema v20 GUI에는 3 frame마다 같은 fresh frame에서 갱신한 YOLO bbox와 CNN
...
ros2 launch xycar_ai_drive traffic_light_viewer.launch.py \
  bundle_id:=traffic-shortcut-nice-ada-very-fast-fix-speed35-regression-resnet18-4s-shadow-ar-handoff-yolo11s-humanbbox-cnn416-actions3-conf50-tl40to225-initial-wait-stop5-go1-stoponce-leftsession1-search3-classify3-vote-yolo3-t500-45sessions-20260824 \
  use_camera:=true
```
...
ros2 run xycar_ai_drive traffic_shortcut_gpu_server -- \
  --bundle-dir /artifacts/traffic-shortcut-nice-ada-very-fast-fix-speed35-regression-resnet18-4s-shadow-ar-handoff-yolo11s-humanbbox-cnn416-actions3-conf50-tl40to225-initial-wait-stop5-go1-stoponce-leftsession1-search3-classify3-vote-yolo3-t500-45sessions-20260824 \
