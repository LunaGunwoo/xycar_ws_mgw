...
`/xycar_motor`를 독점 발행한다. 현재 bundle schema v19는 schema v6 speed-35
nice_ada_very_fast_fix Base와 schema v7 `nice-shortcut` ResNet18, 사람 보정 GT로
재학습한 YOLO11s
...
- YOLO와 CNN은 같은 fresh frame에서 camera sequence 간격 3 이상마다 함께
  ...
  갱신된다. STOP은 같은 raw class 5회, STRAIGHT/LEFT는 첫 fresh 판독 1회에서
  확정한다. fresh `UNKNOWN`과
  no-box는 후보를 0으로 초기화한다. 초기 해제 뒤 STOP은 motion action에서 영구
  무시하지만 pending navigation vote는 초기화한다. schema v19에는 no-box 자동
  정지 해제가 없다.
- 초기 또는 navigation LEFT 첫 fresh 확정 frame에서는 정확히 한 제어 주기
  `[0,0]`을 발행하고, 다음 fresh frame부터 speed `23` ResNet18을 사용한다. 새
  Shortcut prediction을 기다리는 동안 같은 정지 command를 중복 발행하지 않으며
  0.50초를 넘기면 fail-close한다. 동시에 Base는 진입 전 history에서 시작한 self-AR shadow prediction을
  매 fresh frame 계속 갱신하지만 motor에는 발행하지 않는다. 첫 실제 shortcut
  명령부터 8초에 최신 0.50초 이내 shadow command를 즉시 발행해 STOP 없이 Base로
  복귀한다. 성공은 연속 A-hold 활성화당 한 번이다. A를 0.12초 이상 놓았다 다시
  누르면 재무장되며, 더 짧은 입력 누락은 같은 활성화로 취급한다. 같은 활성화의
  두 번째 LEFT에는 `LEFT_IGNORED reason=SESSION_COMPLETE action=RELEASE_A`를
  출력한다. initial phase가 끝난 뒤 STOP은 shortcut과 shadow를 취소하지 않는다.
- 실제 실행 history와 미발행 Base shadow history는 분리한다. schema v19 Base와
  shadow에는 cap `35`가
...
- `use_gamepad:=true`에서는 OFF로 시작하고 A hold/release grace `0.12s` 계약을
  ...
  확정하고, STRAIGHT 첫 fresh 판독은 Base 출발, LEFT 첫 fresh 판독은 shortcut 직행이다.
  A만 누르면 Base로 즉시 출발하고 STOP을 처음부터 무시한다.
  `use_gamepad:=false`도 자동 활성화 직후 동일하게 정지 대기하며 첫 유효 fresh
  class에서 `READY`를 한 번 출력한다. camera/IPC stale, ONNX shape·NaN/Inf,
  ...
  0.50초, IPC timeout은 0.40초다. stale FAULT에는 실제 age와 limit를 출력한다.
- 주행 terminal은 raw class가 바뀌면 즉시, 동일하면 기본 2 Hz로
  `SIGNAL raw=... cnn=... yolo=... source=... phase=... vote=... vote_update=YES/NO stop=...`을
...
- 구독자가 있을 때만 `/traffic_shortcut/signal_debug`에 원본 camera timestamp와
  ...
  fresh/cached source, vote·phase, mission state와 `shortcut_status`를 signal-debug
  schema v2 JSON으로 발행한다. 상태는 `DISABLED`, `READY`, `TRANSITION_STOP`,
  `ACTIVE`, `COMPLETED_THIS_ACTIVATION`, `FAULT` 중 하나다.
...
기존 schema v18 process당 1회 fix-Base GO1, schema v17 기존 Base GO1, schema v16 stop5-go3/search3-classify3, schema v15
initial-wait-all5/search3-classify1, schema v14
initial-stop15/nonstop3, schema v13 speed-35 stop15-go15, schema v12 speed-35
stop10-go30, schema v11 speed-35 stop30-go30, schema v10 speed-25 bundle, schema v9
...
```bash
cd /home/xytron/xycar_ws_mgw && source /opt/ros/humble/setup.bash && source install/setup.bash && ros2 launch xycar_ai_drive jetson_traffic_shortcut.launch.py bundle_id:=traffic-shortcut-nice-ada-very-fast-fix-speed35-regression-resnet18-8s-shadow-ar-handoff-yolo11s-humanbbox-cnn416-actions3-conf50-tl40to225-initial-wait-stop5-go1-stoponce-leftsession1-search3-classify3-vote-yolo3-t500-45sessions-20260824 use_camera:=true use_gamepad:=true allow_motion:=true
```

신호를 기다리려면 `buttons[9]`인 LB를 누른 채 A를 누른다. 차량은 정지한 채
fresh STOP 5회 또는 STRAIGHT/LEFT 첫 fresh 판독을 기다리고 STRAIGHT면 Base,
LEFT면 shortcut으로 출발한다. STOP을 처음부터
무시하고 Base로 바로 출발하려면 A만 누른다. A는 주행 내내 유지하며 LB는 활성화
뒤 놓아도 된다.

대회 연속 주행은 Gamepad를 시작하지 않고 정지 상태에서 신호등을 찾은 뒤
`READY` 뒤 STRAIGHT 또는 LEFT 첫 fresh 판독으로 자동 출발한다.

```bash
cd /home/xytron/xycar_ws_mgw && source /opt/ros/humble/setup.bash && source install/setup.bash && ros2 launch xycar_ai_drive jetson_traffic_shortcut.launch.py bundle_id:=traffic-shortcut-nice-ada-very-fast-fix-speed35-regression-resnet18-8s-shadow-ar-handoff-yolo11s-humanbbox-cnn416-actions3-conf50-tl40to225-initial-wait-stop5-go1-stoponce-leftsession1-search3-classify3-vote-yolo3-t500-45sessions-20260824 use_camera:=true use_gamepad:=false allow_motion:=true
```
...
IPC를 다시 실행하지 않으며 ROS publisher도 만들지 않는다. ROS callback은 Tk와
분리된 background `SingleThreadedExecutor`가 계속 비우고 debug·prediction·motor는
latest-value queue를 사용한다. Tk는 `monitor_refresh_hz` 기본 15 Hz로 snapshot만
렌더하므로 resize 중에도 callback이 쌓이지 않는다. 늦게 도착한 이전 camera
timestamp나 policy frame sequence는 최신 표시 상태를 되돌리지 못한다.

왼쪽 `LIVE CAMERA`는 최신 camera frame만 bbox 없이 보여 준다. 옆의
`MODEL INPUT (EXACT)`는 진단에 실린 ROS timestamp를 최근 30 frame buffer와 exact
match하고, 정확히 일치한 inference frame에만 bbox와 padded CNN crop을 표시한다.
성공한 진단·frame 쌍은 buffer eviction과 별개로 보존한다. 매칭 실패 시 live frame을
대신 쓰지 않고 회색 `FRAME MATCH UNAVAILABLE` placeholder를 표시한다. exact 결과가
1초 이상 오래되면 마지막 class·확률과 함께 `HISTORICAL/STALE`로 명시하며, policy
sequence, timestamp, signal age와 live–model 시간 차이를 함께 표시한다.
...
ros2 run xycar_ai_drive traffic_shortcut_monitor --ros-args \
  ...
  -p bundle_dir:=/home/xytron/xycar_ws_mgw/artifacts/models/traffic-shortcut-nice-ada-very-fast-fix-speed35-regression-resnet18-8s-shadow-ar-handoff-yolo11s-humanbbox-cnn416-actions3-conf50-tl40to225-initial-wait-stop5-go1-stoponce-leftsession1-search3-classify3-vote-yolo3-t500-45sessions-20260824
```

### 수동 신호등 예측 GUI

`traffic_light_viewer`는 schema v4..v19 bundle에서 신호등 YOLO와 CNN classifier만
...
없다. schema v19 GUI에는 3 frame마다 같은 fresh frame에서 갱신한 YOLO bbox와 CNN
padding crop, bundle에 선언된 raw class 확률, bbox 폭 gate, STOP 5회 및
STRAIGHT/LEFT 1회 vote, phase와 최종 `UNKNOWN/RED/LEFT/STRAIGHT`를 함께 표시한다.
`Reset vote / stop latch`는 현재
...
ros2 launch xycar_ai_drive traffic_light_viewer.launch.py \
  bundle_id:=traffic-shortcut-nice-ada-very-fast-fix-speed35-regression-resnet18-8s-shadow-ar-handoff-yolo11s-humanbbox-cnn416-actions3-conf50-tl40to225-initial-wait-stop5-go1-stoponce-leftsession1-search3-classify3-vote-yolo3-t500-45sessions-20260824 \
  use_camera:=true
```
...
ros2 run xycar_ai_drive traffic_shortcut_gpu_server -- \
  --bundle-dir /artifacts/traffic-shortcut-nice-ada-very-fast-fix-speed35-regression-resnet18-8s-shadow-ar-handoff-yolo11s-humanbbox-cnn416-actions3-conf50-tl40to225-initial-wait-stop5-go1-stoponce-leftsession1-search3-classify3-vote-yolo3-t500-45sessions-20260824 \
