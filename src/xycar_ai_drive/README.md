...
`/xycar_motor`를 독점 발행한다. 현재 bundle schema v8은 schema v6 nice_adaptive
...
- YOLO 검출 뒤 중간 frame에서는 직전 bbox를 현재 frame crop에 재사용해 CNN만
  매 frame 실행한다. 동일 `STOP`은 3회(30 FPS 기준 약 0.1초), `LEFT`와
...
- schema v5/v6/v7/v8의 STOP은 YOLO가 confidence `0.25` 이상 box를 10번 연속
...
`traffic_light_viewer`는 schema v4/v5/v6/v7/v8 bundle에서 신호등 YOLO와 CNN classifier만
...
ros2 launch xycar_ai_drive traffic_light_viewer.launch.py \
  bundle_id:=traffic-shortcut-nice-regression-resnet18-8s-shadow-ar-handoff-yolo11s-humanbbox-cnn416-actions3-conf50-tl40to225-stop3-go15-search3-classify1-yolo-miss30-release-45sessions-20260823 \
  use_camera:=true
```
...
ros2 run xycar_ai_drive traffic_light_viewer --ros-args \
  -p bundle_dir:=/home/xytron/xycar_ws_mgw/artifacts/models/traffic-shortcut-nice-regression-resnet18-8s-shadow-ar-handoff-yolo11s-humanbbox-cnn416-actions3-conf50-tl40to225-stop3-go15-search3-classify1-yolo-miss30-release-45sessions-20260823 \
  -p camera_topic:=/image_raw
```
...
ros2 run xycar_ai_drive traffic_shortcut_gpu_server -- \
  --bundle-dir /artifacts/traffic-shortcut-nice-regression-resnet18-8s-shadow-ar-handoff-yolo11s-humanbbox-cnn416-actions3-conf50-tl40to225-stop3-go15-search3-classify1-yolo-miss30-release-45sessions-20260823 \
