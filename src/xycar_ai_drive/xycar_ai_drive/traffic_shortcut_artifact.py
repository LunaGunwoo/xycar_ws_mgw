...
def load_traffic_shortcut_bundle(root: str | Path) -> TrafficShortcutBundle:
    ...
    if (
        ...
        raise ArtifactContractError(
            'traffic shortcut bundle schema must be 1..14'
        )
    ...
    if schema_version in {8, 9, 10, 11, 12, 13, 14}:
        ...
        if detector.get('reuse_detected_bbox_between_yolo_frames') is not True:
            raise ArtifactContractError(
                'schema v8..v14 must reuse the detected bbox between YOLO frames'
            )
        ...
        if (
            ...
            raise ArtifactContractError(
                'adaptive classifier cadence is only valid in schema v8..v14'
            )
    ...
    if (
        schema_version in {6, 7, 8, 9, 10, 11, 12, 13, 14}
    ...
    expected_red_cancels_shortcut = schema_version != 14
    if (
    ...
    expected_base_speed_cap = (
        35.0 if schema_version in {11, 12, 13, 14} else 25.0
    )
