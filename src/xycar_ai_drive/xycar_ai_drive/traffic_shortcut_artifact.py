...
def load_traffic_shortcut_bundle(root: str | Path) -> TrafficShortcutBundle:
    ...
    if schema_version in {8, 9, 10, 11, 12, 13}:
        ...
        if detector.get('reuse_detected_bbox_between_yolo_frames') is not True:
            raise ArtifactContractError(
                'schema v8..v13 must reuse the detected bbox between YOLO frames'
            )
        ...
        if (
            ...
            raise ArtifactContractError(
                'adaptive classifier cadence is only valid in schema v8..v13'
            )
    ...
    expected_base_speed_cap = (
        35.0 if schema_version in {11, 12, 13} else 25.0
    )
