...
class SignalStatusLogGate:
    """Emit immediately on raw-class changes and otherwise at a fixed rate."""

    def __init__(self, *, rate_hz: float) -> None:
        if not math.isfinite(rate_hz) or rate_hz <= 0.0:
            raise ValueError(
                'signal status log rate must be finite and positive'
            )
        self._period_sec = 1.0 / float(rate_hz)
        self.reset()

    def reset(self) -> None:
        self._last_raw_class: SignalClass | None = None
    ...
    def should_emit(
        ...
        *,
        ...
        if not math.isfinite(now_monotonic):
            raise ValueError('signal log timestamp must be finite')
        changed = raw_class != self._last_raw_class
        due = (
            self._last_emit_monotonic is None
            or now_monotonic - self._last_emit_monotonic >= self._period_sec
        )
        if not changed and not due:
            return False
        self._last_raw_class = raw_class
...
def format_signal_status(
    *,
    ...
    yolo = 'n/a' if reading is None else f'{reading.confidence * 100.0:.1f}%'
    vote = '-' if required_reads < 1 else f'{candidate_reads}/{required_reads}'
    return (
...
class TrafficShortcutPolicyNode(Node):
    ...
    def __init__(
        ...
        *,
        ...
        self._runtime_signal_options_enabled = self.bundle.schema_version == 22
        self._effective_bbox_width_min = (
            self.signal_bbox_width_min_px
            if self._runtime_signal_options_enabled
            else self.bundle.detector.bbox_width_min
        )
        self._effective_stop_wait_sec = self.signal_stop_wait_sec
        self._effective_shortcut_duration_sec = (
            self.shortcut_duration_sec
            if self._runtime_signal_options_enabled
            else self.bundle.shortcut_duration_sec
        )
        ...
        self._fsm = TrafficShortcutFsm(
            shortcut_duration_sec=self._effective_shortcut_duration_sec,
            ...
            rearm_shortcut_on_enable=(
                self.bundle.successful_shortcut_once_scope
                == 'drive_gate_activation'
                and not self._runtime_signal_options_enabled
            ),
            repeat_initial_stop_until_shortcut=(
                self._runtime_signal_options_enabled
            ),
        )
        if self._runtime_signal_options_enabled:
            self._lamp_latch = RepeatedSignalEncounterLatch(
                bbox_width_min=self._effective_bbox_width_min,
                bbox_width_max=self.bundle.detector.bbox_width_max,
                stop_wait_sec=self._effective_stop_wait_sec,
                rearm_missing_frames=30,
            )
        elif self.bundle.schema_version in {15, 16, 17, 18, 19, 20, 21}:
            ...
            self._lamp_latch = InitialStopSignalLatch(
                bbox_width_min=self.bundle.detector.bbox_width_min,
                bbox_width_max=self.bundle.detector.bbox_width_max,
                stop_consecutive_reads=(
                    self.bundle.detector.red_consecutive_reads
        ...
        elif self.bundle.detector.mode == 'yolo_cnn_classifier':
        ...
        self._latest_frame: tuple[int, np.ndarray, float, int, int] | None = (
            None
        )
        ...
        self._transition_stop_sent_waiting_decision = False
        self._mission_generation = 0
        self._last_signal = LampAction.UNKNOWN
        self._ready_logged = False
        self._stop_ignored_logged = False
        self._left_ignored_logged = False
        self._shortcut_started_logged = False
        self._signal_log_gate = SignalStatusLogGate(
            rate_hz=self.signal_status_log_hz,
        )
        ...
        startup_message = (
            'Traffic shortcut policy started OFF. Hold LB+A to approach on '
            'Base, stop at the configured YOLO width, wait before accepting '
            'one fresh class, and repeat after STRAIGHT until LEFT succeeds; '
            'or hold A alone to bypass encounter stops.'
            if self.bundle.schema_version == 22
            else 'Traffic shortcut policy started OFF. Hold LB+A to wait stopped '
            'for 1 fresh STRAIGHT/LEFT or 5 STOP classifications, '
            'or hold A alone to start Base immediately; release A to stop.'
            if self.bundle.schema_version in {17, 18, 19, 20, 21, 22}
            else 'Traffic shortcut policy started OFF. Hold LB+A to wait stopped '
            'for 3 matching fresh STRAIGHT/LEFT or 5 STOP classifications, '
            'or hold A alone to start Base immediately; release A to stop.'
        ...
        self.get_logger().warning(startup_message)
        self.get_logger().info(
            f'bundle={self.bundle.artifact_id}, base_cap='
            f'{self.bundle.base_speed_cap:g}, shortcut_speed='
            f'{self.bundle.shortcut_speed:g}, bundle_bbox_width='
            f'[{self.bundle.detector.bbox_width_min},'
            f'{self.bundle.detector.bbox_width_max}], effective_bbox_width='
            f'[{self._effective_bbox_width_min},'
            f'{self.bundle.detector.bbox_width_max}], bundle_shortcut_duration='
            f'{self.bundle.shortcut_duration_sec:g}s, '
            f'effective_shortcut_duration='
            f'{self._effective_shortcut_duration_sec:g}s, '
            f'signal_stop_wait={self._effective_stop_wait_sec:g}s, '
    ...
    def _declare_parameters(self) -> None:
        ...
        self.declare_parameter('signal_bbox_width_min_px', 40)
        self.declare_parameter('signal_stop_wait_sec', 1.0)
        self.declare_parameter('shortcut_duration_sec', 5.0)
        ...
        self.declare_parameter('inference_timeout_sec', 0.50)
        ...
        self.declare_parameter('inference_rpc_timeout_sec', 0.40)
    ...
    def _read_parameters(self) -> None:
        ...
        self.signal_status_log_hz = float(
            self.get_parameter('signal_status_log_hz').value
        )
        self.signal_bbox_width_min_px = int(
            self.get_parameter('signal_bbox_width_min_px').value
        )
        self.signal_stop_wait_sec = float(
            self.get_parameter('signal_stop_wait_sec').value
        )
        self.shortcut_duration_sec = float(
            self.get_parameter('shortcut_duration_sec').value
        )
    ...
    def _validate_parameters(self) -> None:
        ...
        if self.a_button_index < 0 or self.initial_stop_arm_button_index < 0:
            raise ValueError('gamepad button indexes must be non-negative')
        if self.a_button_index == self.initial_stop_arm_button_index:
            raise ValueError('A and initial STOP arm buttons must differ')
        ...
        if not 1 <= self.signal_bbox_width_min_px <= 225:
            raise ValueError(
                'signal_bbox_width_min_px must be in [1,225]'
            )
        for label, value in (
            ('a_release_grace_sec', self.a_release_grace_sec),
            ('signal_status_log_hz', self.signal_status_log_hz),
            ('signal_stop_wait_sec', self.signal_stop_wait_sec),
            ('shortcut_duration_sec', self.shortcut_duration_sec),
    ...
    def _validate_bundle_runtime_contract(self) -> None:
        ...
        if (
            ...
            raise ValueError(
                'traffic detector width/votes/every contract mismatch'
            )
        ...
        expected_yolo_missing_release_frames = (
            30 if 5 <= self.bundle.schema_version <= 13 else None
        )
        if (
        ...
        if self.bundle.schema_version == 14:
            if (
                ...
                or self.bundle.initial_stop_clear_consecutive_reads != 3
            ...
            if (
                ...
                or self.bundle.initial_stop_requires_stop_confirmation
            ):
        ...
        elif self.bundle.schema_version == 16:
            if (
                not self.bundle.initial_stop_one_shot
                or self.bundle.initial_stop_clear_consecutive_reads != 3
                or not self.bundle.headless_wait_for_first_signal
                ...
                raise ValueError('schema v16 initial wait contract mismatch')
        elif self.bundle.schema_version in {17, 18, 19, 20, 21}:
            if (
                not self.bundle.initial_stop_one_shot
                or self.bundle.initial_stop_clear_consecutive_reads != 1
                or not self.bundle.headless_wait_for_first_signal
                or not self.bundle.initial_left_direct_shortcut
                or not self.bundle.control_vote_on_fresh_yolo_only
                or self.bundle.initial_stop_requires_stop_confirmation
                ...
                raise ValueError(
                    'schema v17..v21 initial wait contract mismatch'
                )
        elif self.bundle.schema_version == 22:
            if (
                not self.bundle.initial_stop_one_shot
                or self.bundle.initial_stop_clear_consecutive_reads != 1
                or not self.bundle.headless_wait_for_first_signal
                or not self.bundle.initial_left_direct_shortcut
                or not self.bundle.control_vote_on_fresh_yolo_only
                or not self.bundle.initial_stop_requires_stop_confirmation
            ):
                raise ValueError('schema v22 bundle contract mismatch')
        elif (
        ...
        expected_shortcut_scope = (
            'drive_gate_activation'
            if self.bundle.schema_version in {19, 20, 21, 22}
            else 'process'
        )
        if (
            self.bundle.successful_shortcut_once_scope
            ...
            raise ValueError('shortcut once scope contract mismatch')
        if (
    ...
    def _validate_client_artifacts(self) -> None:
        ...
        if (
            ...
            raise ValueError(
                'shortcut IPC client artifact does not match bundle'
            )
    ...
    def _initial_external_history(self):
        ...
        if history is None or history.update != 'externally_executed_commands':
            raise ValueError(
                'Base artifact must use external executed history'
            )
        return deque(
    ...
    def _on_joy(self, message: Joy) -> None:
        ...
        if len(message.buttons) <= required_button_index:
            ...
            self._force_fault('Joy button array is too short for A/LB')
            return
        pressed = bool(message.buttons[self.a_button_index])
        initial_stop_requested = self.bundle.schema_version in {
            14,
            15,
            16,
            17,
        ...
        with self._lock:
            ...
            initial_stop_armed = initial_stop_requested and not (
                self._runtime_signal_options_enabled
                and self._fsm.shortcut_completed
            )
            action = self._observe_drive_gate_locked(
                ...
                wait_for_signal=(
                    initial_stop_armed
                    and self.bundle.schema_version
                    in {15, 16, 17, 18, 19, 20, 21}
                ),
            )
        if action == ToggleAction.ENABLED:
            if (
                self.bundle.schema_version in {15, 16, 17, 18, 19, 20, 21, 22}
                and initial_stop_armed
            ):
                message = (
                    'Traffic shortcut repeated signal handling enabled by '
                    'LB+A; approaching on Base until the configured YOLO width.'
                    if self.bundle.schema_version == 22
                    else 'Traffic shortcut signal wait enabled by LB+A; motor '
                    'remains stopped.'
                )
                self.get_logger().warning(message)
                self._safe_log_warning('INITIAL_STOP_ARMED source=LB+A')
                self._safe_log_warning(
                    'SIGNAL_APPROACH source=LB+A'
                    if self.bundle.schema_version == 22
                    else 'WAIT_FOR_SIGNAL source=LB+A'
                )
            else:
        ...
        *,
        ...
        if (
            ...
            and (
                ...
                and (
                    not self.bundle.initial_stop_requires_stop_confirmation
                    or raw == SignalClass.STOP
                )
                and not self._ready_logged
                and (self.bundle.schema_version != 15 or vote_updated)
            )
        ):
        ...
        if isinstance(
            ...
            if (
                ...
                and current.phase == InitialStopPhase.WAIT_FOR_SIGNAL
            ):
                messages.append(
                    'SIGNAL_STOP_TRIGGER '
                    f'bbox_min={self._effective_bbox_width_min}px '
                    f'wait={self._effective_stop_wait_sec:g}s'
                )
            if (
                previous.phase == InitialStopPhase.WAIT_FOR_SIGNAL
                and current.phase != InitialStopPhase.WAIT_FOR_SIGNAL
            ):
                messages.append('SIGNAL_DWELL_COMPLETE')
            if (
                previous.phase == InitialStopPhase.PASSED
                and current.phase == InitialStopPhase.ARMED
            ):
                messages.append('SIGNAL_REARMED missing_frames=30')
        if (
            not isinstance(
                self._lamp_latch,
                RepeatedSignalEncounterLatch,
        ...
        if vote_updated and previous.phase in stopped_phases:
            ...
            elif current.candidate_reads > 0 and raw in {
                SignalClass.STRAIGHT,
                SignalClass.LEFT,
            }:
                messages.append(
                    f'NON_STOP_CLEAR {current.candidate_reads}/{current.required_reads}'
                )
        stop_ignored = (
            vote_updated
        ...
        left_ignored = (
            vote_updated
            and signal == LampAction.LEFT
            and plan.state == MissionState.BASE
            and getattr(
                getattr(self, '_fsm', None),
                'shortcut_completed',
                False,
            )
        )
        if left_ignored:
            if not getattr(self, '_left_ignored_logged', False):
                self._left_ignored_logged = True
                messages.append(
                    'LEFT_IGNORED reason=SESSION_COMPLETE action=RELEASE_A'
                )
        else:
            self._left_ignored_logged = False
        if self._signal_log_gate.should_emit(
            ...
            stop_status = {
                InitialStopPhase.ARMED: 'ARMED',
                InitialStopPhase.WAIT_FOR_SIGNAL: 'LATCHED',
                InitialStopPhase.STOPPED: 'LATCHED',
                InitialStopPhase.NAVIGATION: 'IGNORED',
                InitialStopPhase.PASSED: 'PASSED',
            }[current.phase]
            messages.append(
    ...
    def _emit_operator_logs(self, messages: Sequence[str]) -> None:
    ...
    def _shortcut_status_locked(self, mission_state: MissionState) -> str:
        if mission_state == MissionState.OFF:
            return 'DISABLED'
        if mission_state == MissionState.FAULT:
            return 'FAULT'
        if mission_state == MissionState.SWITCH_TO_SHORTCUT:
            return 'TRANSITION_STOP'
        if mission_state in {
            MissionState.SHORTCUT,
            MissionState.SWITCH_TO_BASE,
        }:
            return 'ACTIVE'
        if self._fsm.shortcut_completed:
            return 'COMPLETED_THIS_ACTIVATION'
        return 'READY'
    ...
    def _make_signal_debug_snapshot_locked(
        ...
        *,
        ...
        return SignalDebugSnapshot(
            ...
            class_labels=tuple(self.bundle.detector.classifier_classes),
            ...
            shortcut_status=self._shortcut_status_locked(plan.state),
                ...
                if inference_plan.run_detector:
                    ...
                    if self.bundle.detector.mode == 'yolo_cnn_classifier':
                        ...
                        if inspection is not None:
                            ...
                            inference_source = 'YOLO_CNN'
                        else:
                            inference_source = 'YOLO_NO_BOX'
                    else:
                ...
                with self._lock:
                    ...
                    if vote_updated:
                        ...
                        if isinstance(
                            ...
                            signal = self._lamp_latch.observe(
                                reading,
                                now_monotonic=observation_now,
                                detector_frame_span=(
                                    inference_plan.detector_frame_span
                                ),
                            )
                            if (
                                self._lamp_latch.rearmed_on_last_observation
                            ):
                                self._fsm.rearm_initial_stop()
                        else:
                    ...
                    if self._yolo_missing_release is not None:
                        release_after_yolo_loss = (
                            self._yolo_missing_release.observe(
                                red_stop_active=(
                                    self._fsm.state == MissionState.RED_STOP
                                ),
                    ...
                    if (
                        reading_observed
                        and self.bundle.detector.mode == 'yolo_cnn_classifier'
                    ):
                    ...
                    if (
                        ...
                        self._shortcut_started_logged = True
                    ...
                    if plan.publish_stop:
                        if plan.state == MissionState.SWITCH_TO_SHORTCUT:
                            ...
                            self._decision = None
                            shadow_work = self._start_base_shadow_locked()
                        else:
                            self._discard_base_shadow_locked()
                        ...
                        if plan.policy == PolicyChoice.BASE:
                            ...
                            shadow_work = self._base_shadow_snapshot_locked()
                        generation = self._mission_generation
    ...
    def _on_control_timer(self) -> None:
        ...
        auto_action = None
        transition_stop_already_sent = False
        with self._lock:
            if (
            ...
            if reason is None and self._drive_gate.enabled:
                deadline_plan = self._fsm.on_control_tick(now_monotonic=now)
                ...
                if reason is None and self._transition_stop_pending:
                    ...
                    self._emit_operator_logs(('TRANSITION_STOP cycle=1/1',))
                    self._transition_stop_pending = False
                    self._transition_stop_sent_waiting_decision = True
                    self._mission_generation += 1
                    self._minimum_next_frame_sequence = self._frame_sequence
                    self._decision = None
                    self._awaiting_post_reset_decision = True
                ...
                if reason is None and decision is not None:
                    ...
                    self._stop_reason = None
                    return
            transition_stop_already_sent = (
                self._transition_stop_sent_waiting_decision
            )
        if auto_action == ToggleAction.ENABLED:
            ...
            message = (
                ...
                'or 5 STOP classifications.'
                if self.bundle.schema_version in {17, 18, 19, 20, 21}
                else 'Traffic shortcut headless signal search started; motor '
                'remains stopped until READY and 3 matching fresh '
                'STRAIGHT/LEFT or 5 STOP classifications.'
                if self.bundle.schema_version == 16
                else 'Traffic shortcut headless signal search started; motor '
                'remains stopped until READY and matching fresh YOLO '
                'classifications.'
                if self.bundle.schema_version == 15
                else 'Traffic shortcut headless signal search started; motor '
            ...
            self._force_fault(
                self._stop_reason or 'automatic traffic shortcut start failed'
            )
            return
    ...
    def _promote_base_shadow_locked(self, now: float) -> str | None:
        ...
        if not math.isfinite(age) or age < 0.0 or age > max_age:
            return f'Base shadow prediction is stale at shortcut handoff: age={age:.3f}s'
    ...
    def _unsafe_reason_locked(self, now: float) -> str | None:
        ...
        if (
            self.bundle.initial_stop_requires_stop_confirmation
            and self._fsm.state
            in {MissionState.WAIT_FOR_SIGNAL, MissionState.INITIAL_STOP}
        ):
            # No motion decision is selected during the intentional signal
            # stop. Camera/graph faults above still fail closed, while slow
            # fresh YOLO classification cannot convert a safe wait to FAULT.
            return None
        if self._awaiting_post_reset_decision:
            if self._history_reset_monotonic is None:
                return 'post-reset mission inference is missing'
            reset_age = now - self._history_reset_monotonic
            if reset_age > self.inference_timeout_sec:
                return (
                    'post-reset mission inference is stale: '
                    f'age={reset_age:.3f}s '
                    f'limit={self.inference_timeout_sec:.3f}s'
                )
            return None
        if self._drive_gate.enabled and not self._transition_stop_pending:
            if self._decision is None:
                return 'selected policy inference is missing'
            decision_age = now - self._decision.source_monotonic
            if decision_age > self.inference_timeout_sec:
                return (
                    'selected policy inference is stale: '
                    f'age={decision_age:.3f}s '
                    f'limit={self.inference_timeout_sec:.3f}s'
                )
        return None
    ...
    def _force_fault(self, reason: str) -> None:
        ...
        if changed_reason:
            self.get_logger().warning(
                f'Traffic shortcut motion stopped: {reason}'
            )
