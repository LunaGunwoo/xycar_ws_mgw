...
class SignalStatusLogGate:
    ...
    def __init__(self, *, rate_hz: float) -> None:
        if not math.isfinite(rate_hz) or rate_hz <= 0.0:
            raise ValueError('signal status log rate must be finite and positive')
    ...
    def should_emit(
        ...
        *,
        ...
        due = (
            ...
            or now_monotonic - self._last_emit_monotonic
            >= self._period_sec
        )
...
def format_signal_status(
    *,
    ...
    vote = (
        '-'
        if required_reads < 1
        else f'{candidate_reads}/{required_reads}'
    )
    return (
...
class TrafficShortcutPolicyNode(Node):
    ...
    def __init__(
        ...
        *,
        ...
        if self.bundle.schema_version in {15, 16, 17}:
        ...
        startup_message = (
            ...
            if self.bundle.schema_version == 17
    ...
    def _validate_bundle_runtime_contract(self) -> None:
        ...
        expected_base_speed_cap = (
            35.0
            if self.bundle.schema_version in {11, 12, 13, 14, 15, 16, 17}
            else 25.0
        )
        ...
        elif self.bundle.schema_version == 17:
            if (
                not self.bundle.initial_stop_one_shot
                ...
                raise ValueError('schema v17 initial wait contract mismatch')
        elif (
    ...
    def _on_joy(self, message: Joy) -> None:
        ...
        required_button_index = (
            ...
            if self.bundle.schema_version in {14, 15, 16, 17}
        ...
        initial_stop_armed = (
            self.bundle.schema_version in {14, 15, 16, 17}
        ...
        with self._lock:
            ...
            action = self._observe_drive_gate_locked(
                ...
                wait_for_signal=(
                    initial_stop_armed
                    and self.bundle.schema_version in {15, 16, 17}
                ),
            )
        if action == ToggleAction.ENABLED:
            if (
                self.bundle.schema_version in {15, 16, 17}
            ...
            try:
                ...
                with self._lock:
                    ...
                    if (
                        ...
                        operator_messages.append('SHORTCUT_STARTED duration=8.0s')
